    ...
    if not skip_rbf:
        if freqs is None:
            if bandwidth is None:
                ...
                bandwidth = pick_gaussian_bandwidth(stats, skip_feats=skip_feats)
...
def _my_proc_setup(stats, skip_feats, common_feats, do_check=True):
    ...
    if new_pumas:
        ...
        vc["OCCP"] = (
            vc["occp02"].groupby(occ_cat).sum() + vc["occp10"].groupby(occ_cat).sum()
        )
    ...
    vc["ETHNICITY"] = pd.Series(
        [stats["n_total"] - 5] + [1] * 5,
        index=["hispanic", "white", "black", "amerindian", "asian", "other/biracial"],
    )
    # obviously the value counts are a lie, but we don't actually use them

    stats["value_counts"] = OrderedDict(
        sorted((k, v.sort_index()) for k, v in vc.items())
    )
...
def _my_proc_chunk(df, stats, skip_feats=set()):
    ...
    if "ETHNICITY" not in skip_feats:
        df["ETHNICITY"] = df.RAC1P.where(df.HISP == 1, "hispanic").map(_ethnicity_map)
...
def my_additive_setup(stats, skip_feats, seed):
    ...
    m.rff_discrete_pairs = [(r, d) for r in m.rff_reals for d in m.discretes]
    ...
    m.one_bws = [_get_median(samp[:, [i]]) for i in range(len(m.rff_reals))]
    m.pair_bws = [
    ...
    m.one_freqs = [
        pick_rff_freqs(m.one_n_freqs, bandwidth=bw, n_feats=1, seed=rs.randint(2 ** 23))
        for bw in m.one_bws
    ]
    m.one_freqs_d = {n: f for n, f in zip(m.rff_reals, m.one_freqs)}
    m.pair_freqs = [
        pick_rff_freqs(
            m.pair_n_freqs, bandwidth=bw, n_feats=2, seed=rs.randint(2 ** 23)
        )
...
def my_additive_extras(feats, wts, m, feat_locs, out=None):
    ...
    for (f1, f2), freqs in zip(m.rff_pairs, m.pair_freqs):
        rff_embedding(
            feats[:, feat_locs[f1] + feat_locs[f2]],
            wts,
            freqs,
